...
| **Versão** | 1.0 |
| **Data** | 31/07/2026 |
| **Banco** | PostgreSQL 16 · ORM Entity Framework Core (provider Npgsql) |
...
| Convenção | Regra |
|---|---|
| Nomes | `snake_case` no banco, `PascalCase` no código (mapeado por `IEntityTypeConfiguration<T>` do EF Core) |
...
| Configuração flexível | `JSONB` com schema validado por FluentValidation na aplicação |
...
Interceptor de conexão do EF Core, antes de qualquer query (`TenantConnectionInterceptor`, ver ADR-038):
...
| Regra | Motivo |
|---|---|
| Toda alteração de schema é uma migration do EF Core (`dotnet ef migrations add`) versionada | Reprodutibilidade em N lojas |
