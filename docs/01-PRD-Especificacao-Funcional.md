...
| RF-PLT-09 | O admin da plataforma deve dispor de uma área autenticada com página inicial e navegação para as funções globais | M | 5 |
| RF-PLT-10 | O admin da plataforma deve listar, buscar, filtrar e ordenar todos os estabelecimentos autorizados | M | 5 |
| RF-PLT-11 | O admin da plataforma deve consultar uma visão administrativa consolidada de cada estabelecimento e seus acessos disponíveis | M | 5 |
| RF-PLT-12 | O sistema deve controlar ativação, suspensão, reativação e cancelamento por máquina de estados auditável | M | 5 |
| RF-PLT-13 | O sistema deve manter catálogo de planos, plano vigente e histórico de alterações por estabelecimento | S | 5 |
| RF-PLT-14 | O admin da plataforma deve acompanhar proprietário, usuários iniciais e convites, incluindo recuperação segura do acesso | M | 5 |
| RF-PLT-15 | O admin da plataforma deve recuperar provisionamento incompleto e reemitir credencial de instalação sem duplicar o tenant | M | 5 |
| RF-PLT-16 | O admin da plataforma deve visualizar pendências priorizadas, histórico administrativo e atalhos autorizados de saúde e suporte | M | 5 |
