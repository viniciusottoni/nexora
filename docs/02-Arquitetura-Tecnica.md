...
```
                          ...
                          │              NUVEM                    │
                          │  ┌─────────────────────────────────┐  │
   Internet               │  │  API Cloud (ASP.NET Core)       │  │
   ─────────────────────► │  │  · Sync gateway                 │  │
...
> Decisões formalizadas em ADR-001, ADR-036 a ADR-039 e ADR-004 a ADR-014 (as que não dependem de framework). Aqui está o resultado consolidado. O backend foi migrado de Node/NestJS/Prisma para C#/.NET seguindo a arquitetura de referência do projeto irmão `seminarioteologico` (Clean Architecture + CQRS/MediatR); o frontend não muda.

### 3.1 Visão geral

| Camada | Tecnologia | Motivo em uma linha |
|---|---|---|
| **Linguagem (backend)** | C# (.NET 10) | Tipagem forte, fronteiras entre camadas impostas pelo compilador |
| **Estrutura de projeto** | Solution única (`.slnx`) + `Directory.Build.props` + `Directory.Packages.props` | Código compartilhado entre local, nuvem e testes; versionamento central de pacotes |
| **Backend** | ASP.NET Core Web API (.NET 10) | Mesmo domínio roda local e na nuvem; `ProblemDetails` nativo (RFC 7807) |
| **Padrão de aplicação** | Clean Architecture + CQRS com MediatR | Controllers finos; commands/queries isolam escrita de leitura (ver `ARCHITECTURE.md` do `seminarioteologico`) |
| **Validação** | FluentValidation | Validators por command/query, plugados no `ValidationBehavior` do MediatR |
| **ORM** | Entity Framework Core (provider Npgsql) | Migrations versionadas, tipagem forte, mantém PostgreSQL + RLS |
| **Banco** | PostgreSQL 16 | RLS nativo para multi-tenant; JSONB para configuração; robusto offline |
| **Cache/Fila** | Redis (StackExchange.Redis) + `BackgroundService` | Sync worker, agendamentos, rate limit |
| **Realtime** | SignalR sobre WebSocket | Reconexão automática, grupos por tenant/praça |
| **Frontend** | React 18 + Vite + TypeScript | PWA maduro, ecossistema, curva de time — **inalterado** |
...
| **Auth** | JWT Bearer (access curto + refresh) + PIN para operação | Perfis distintos exigem métodos distintos |
| **Armazenamento** | Object storage S3-compatível | Fotos de produto, logos, exportações |
| **Containers** | Docker + Docker Compose (loja) | Instalação padronizada e replicável; imagem base `mcr.microsoft.com/dotnet/aspnet` |
| **Observabilidade** | OpenTelemetry (.NET SDK) + Sentry + logs estruturados (Serilog) | Suporte remoto a N instalações |
| **Testes** | xUnit, `WebApplicationFactory`, Testcontainers.PostgreSql, Playwright, k6 | Pirâmide completa (doc. 10) |
| **CI/CD** | GitHub Actions | `dotnet build`/`test`/`publish`, imagens, migrations |

### 3.2 Estrutura da solution

```
nexora/
├── backend/
│   ├── src/
│   │   ├── Nexora.Api.Cloud/          ASP.NET Core — nuvem (sync, BI, financeiro, admin)
│   │   ├── Nexora.Api.Edge/           ASP.NET Core — servidor da loja (pedido, KDS, caixa)
│   │   ├── Nexora.Application/        Commands/Queries/Handlers (CQRS+MediatR), FluentValidation, Behaviors
│   │   ├── Nexora.Domain/             Entidades, máquinas de estado, regras puras — zero dependências
│   │   ├── Nexora.Infrastructure/     EF Core (Npgsql), interceptor de RLS, Redis, SignalR hubs, storage
│   │   ├── Nexora.Contracts/          DTOs de request/response compartilhados Api↔Application
│   │   └── Nexora.Shared/             Constantes de erro, helpers sem regra de negócio
│   ├── tests/
│   │   ├── Nexora.UnitTests/          Domain e handlers, sem infraestrutura
│   │   ├── Nexora.IntegrationTests/   EF Core + Testcontainers.PostgreSql (valida RLS real)
│   │   ├── Nexora.ApiTests/           WebApplicationFactory — ponta a ponta HTTP
│   │   └── Nexora.ArchitectureTests/  NetArchTest — garante fronteiras entre projetos
...
│       ├── contracts/       Tipos TS gerados a partir do contrato OpenAPI do backend
...
> **Ponto central:** `Nexora.Domain` contém as regras de negócio puras, sem dependência de framework — nem MediatR, nem EF Core, nem ASP.NET Core — e é referenciado **igualmente** por `Api.Edge` e `Api.Cloud`. A ausência de `ProjectReference`/`PackageReference` é verificada em tempo de compilação e reforçada por `Nexora.ArchitectureTests`. É isso que garante que o comportamento offline seja idêntico ao online — a mesma garantia do stack anterior, só que agora imposta pelo compilador em vez do linter (ver [ADR-039](ADRs/ADR-039-fronteiras-por-project-reference.md)).
...
| **Autorização** | RBAC por tenant, verificado em `[Authorize]`/policy do ASP.NET Core + RLS no banco |
...
services:
  ...
  api-edge:   # ASP.NET Core: API + SignalR Hub
...
```
Sprint 0   Solution .NET, CI, EF Core, RLS, autenticação, tenant, docker-compose edge
