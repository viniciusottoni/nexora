...
| EVT-056 | `tenant.status_changed` | Ciclo de vida administrativo alterado | ↓ | 5 |
| EVT-057 | `tenant.plan_changed` | Plano comercial entrou em vigência | ↓ | 5 |
| EVT-058 | `tenant.owner_access_changed` | Convite ou titularidade do proprietário mudou | ↓ | 5 |
| EVT-059 | `installation.token_reissued` | Credencial de instalação foi rotacionada | — | 5 |
...
- [ ] Schema definido como `record` imutável em `Nexora.Domain` (ou `Nexora.Contracts`, quando cruza a fronteira HTTP/sync) e versionado
