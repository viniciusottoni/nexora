...
```
                    ...
                    ├─────────────────┤
                    │   Integração    │   ~200 (WebApplicationFactory + Testcontainers.PostgreSql)
                    ├─────────────────┤
                    │    Unitário     │   ~800 (xUnit)
                    └─────────────────┘
...
|---|---|---|---|
| Unitário | ≥ 70% global · ≥ 90% em `Nexora.Domain` | 60 s | Todo commit |
...
Foco em `Nexora.Domain` — regras puras, sem banco e sem framework.
...
```csharp
public class PrecificacaoDeMeioAMeioTests
{
    [Fact]
    public void AplicaOMaiorValorQuandoARegraEhHighest()
    {
        var item = OrderItemBuilder.Build(fractions:
        [
            new Fraction(VariantBuilder.Build(price: 45m), weight: 0.5m),
            new Fraction(VariantBuilder.Build(price: 52m), weight: 0.5m),
        ]);

        var price = PriceCalculator.Calculate(item, new PricingOptions { HalfAndHalfPricing = HalfPricing.Highest });

        price.Should().Be(52m);
    }

    [Fact]
    public void AplicaAMediaQuandoARegraEhAverage()
    {
        // ... espera 48.50
    }
}

public class MaquinaDeEstadosDoItemTests
{
    [Fact]
    public void NaoPermiteCancelarItemIniciadoSemAutorizacao()
    {
        var item = OrderItemBuilder.Build(status: OrderItemStatus.Fired);

        var act = () => item.Cancel(authorization: null);

        act.Should().Throw<AuthorizationRequiredException>();
    }

    [Fact]
    public void NaoPermiteRetrocederDeReadyParaFired()
    {
        var item = OrderItemBuilder.Build(status: OrderItemStatus.Ready);

        var act = () => item.TransitionTo(OrderItemStatus.Fired);

        act.Should().Throw<InvalidTransitionException>();
    }
}

public class BaixaDeEstoqueEmMeioAMeioTests
{
    [Fact]
    public void BaixaMetadeDosInsumosDeCadaFicha()
    {
        var movements = StockDeduction.Deduct(_halfAndHalfItem);

        movements.Should().ContainEquivalentOf(
            new StockMovement(ingredient: "mussarela", quantity: 0.09m)); // metade de 0,18
    }
}
```
...
Contra **PostgreSQL real** via **Testcontainers.PostgreSql** (nunca mock de banco — RLS não pode ser simulado).
...
```csharp
public class IntegridadeDeInstrumentacaoTests : IClassFixture<PostgresTestcontainerFixture>
{
    [Fact]
    public async Task TodaTransicaoDeEstadoEmiteSeuEvento()
    {
        var order = await CreateOrderAsync(...);
        await AdvanceItemAsync(order.Items[0].Id, OrderItemStatus.Fired);

        var events = await FindEventsAsync(aggregateId: order.Items[0].Id);

        events.Should().ContainSingle(e =>
            e.Type == "order.item.fired" &&
            e.ActorId != null &&
            e.DeviceId != null &&
            e.OccurredAt != default);
    }

    [Fact]
    public async Task EventoEEstadoSaoGravadosNaMesmaTransacao()
    {
        await SimulateFailureAfterStateWriteAsync();

        // nem estado nem evento devem ter sido persistidos
        (await FindOrderAsync(id)).Should().BeNull();
        (await FindEventsAsync(aggregateId: id)).Should().BeEmpty();
    }
}
```
...
```csharp
public class IsolamentoEntreTenantsTests : IClassFixture<ApiTestFixture>
{
    [Theory]
    [MemberData(nameof(AllBusinessTables))]
    public async Task TabelaNaoVazaEntreTenants(string table)
    {
        var tenantA = await CreateTenantWithDataAsync();
        var tenantB = await CreateTenantWithDataAsync();

        await using var connection = await OpenConnectionAsTenantAsync(tenantA.Id);
        var rows = await connection.QueryAsync($"SELECT * FROM {table}");

        rows.Should().OnlyContain(r => r.tenant_id == tenantA.Id);
    }

    [Fact]
    public async Task AcessoDiretoPorIdDeOutroTenantRetorna404()
    {
        var response = await _client.As(_userOfTenantA).GetAsync($"/v1/orders/{_orderOfTenantB.Id}");

        response.StatusCode.Should().Be(HttpStatusCode.NotFound); // idêntico a inexistente
        (await FindAuditLogAsync(action: "CROSS_TENANT_ATTEMPT")).Should().NotBeNull();
    }

    [Fact]
    public async Task QuerySemContextoDeTenantNaoRetornaNada()
    {
        await using var db = CreateDbContextWithoutTenantContext();
...
`ApiTestFixture` encapsula `WebApplicationFactory<Program>` (padrão do `seminarioteologico`, ver classe `LoginApiFactory`) e `PostgresTestcontainerFixture` (`Testcontainers.PostgreSql`).
...
```csharp
[Fact]
public async Task D06_MetricaUsaHorarioDeOcorrenciaNaoDeSincronizacao()
{
    var @event = await CreateEventOfflineAsync(
        type: "order.placed",
        occurredAt: new DateTimeOffset(2026, 7, 31, 20, 3, 0, TimeSpan.Zero));

    await SyncAtAsync(new DateTimeOffset(2026, 7, 31, 21, 15, 0, TimeSpan.Zero));

    var metrics = await GetHourlyMetricsAsync("2026-07-31");

    metrics.Single(m => m.Hour == "20:00").Orders.Should().Be(1);
    (metrics.SingleOrDefault(m => m.Hour == "21:00")?.Orders ?? 0).Should().Be(0);
}
```
...
```
Commit
  ├─ dotnet format + analyzers   (10 s)   bloqueante
  ├─ dotnet build (type check)   (30 s)   bloqueante
  └─ Unitários (dotnet test)     (60 s)   bloqueante

Pull Request
  ├─ Integração (Testcontainers) (5 min)  bloqueante
  ├─ ISOLAMENTO MULTI-TENANT     (30 s)   bloqueante
  ├─ Verificação ADR-013         (5 s)    bloqueante
  ├─ Nexora.ArchitectureTests (10 s) bloqueante
