...
| # | Princípio |
|---|---|
| 1 | **Duas APIs, um contrato.** `api-edge` (loja) e `api-cloud` compartilham DTOs de `packages/contracts`. O front não sabe com quem fala. |
...
{ "type": "alert.raised",
  "data": { "alertType": "ORDER_LATE", "severity": "HIGH", "entityId": "...", "message": "..." } }
...
| Item | Ferramenta |
|---|---|
| Especificação | OpenAPI 3.1 gerado por decorators do NestJS |
| Publicação | `/docs` em ambiente não produtivo |
| Tipos do cliente | Gerados de `packages/contracts` — front e back nunca divergem |
| Validação | Zod nos DTOs, tanto na entrada quanto na saída |
