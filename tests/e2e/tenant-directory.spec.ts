...
test.describe('Diretório de estabelecimentos com busca e filtros (US-151)', () => {
  test('listagem inicial ordenada por criticidade, e cada linha abre o detalhe do estabelecimento', async ({
    ...
    await page.route('**/v1/platform/tenants**', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({ data: [donaBetinha, hamburgueriaDoZe], nextCursor: null, appliedFilters: appliedFilters() }),
      });
    });
  ...
  test('base grande: paginação por cursor avança sem repetir nem omitir registros', async ({ page }) => {
    await mockLogin(page, adminSession);
    await mockPlatformSummary(page);
    ...
    await page.route('**/v1/platform/tenants**', async (route) => {
      ...
      if (cursor === 'cursor-pagina-2') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ data: [pageTwoTenant], nextCursor: null, appliedFilters: appliedFilters() }),
        });
        return;
      }
  ...
  test('usuário sem policy PlatformAdmin recebe 403 e nenhum dado do diretório é exposto', async ({ page }) => {
