...
internal sealed class ActivateVariantCommandHandler : IRequestHandler<ActivateVariantCommand, Result<VariantResponse>>
{
    private readonly IApplicationDbContext _db;
    private readonly ICurrentTenantContext _tenantContext;

    public ActivateVariantCommandHandler(IApplicationDbContext db, ICurrentTenantContext tenantContext)
    {
        _db = db;
        _tenantContext = tenantContext;
    }

    public async Task<Result<VariantResponse>> Handle(ActivateVariantCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!variant.IsActive)
        {
            ...
            _db.AuditLogs.Add(AuditLog.Create(
                tenantId, action: "PRODUCT_VARIANT_ACTIVATED", entity: "product_variant",
