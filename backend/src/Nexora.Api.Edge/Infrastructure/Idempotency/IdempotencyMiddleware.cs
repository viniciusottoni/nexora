...
namespace Nexora.Api.Edge.Infrastructure.Idempotency;
...
/// resolvido quando a requisição chega — pareamento de dispositivo (anônimo por design, ADR-004),
/// provisionamento de tenant (ação de plataforma, tenant nem existe ainda) e registro de
/// instalação — escopar por tenant tornaria a chave inutilizável exatamente nesses casos; (2) a
/// chave já É globalmente única por construção (o cliente gera um UUID aleatório por intenção —
/// ver packages/ui/src/auth/operational-auth-client.ts, apps/web-admin/*-api.ts), então uma
...
/// </para>
/// <para>
/// <b>Rotas isentas:</b> ver <see cref="IdempotencyExemptAttribute"/> — hoje login por PIN
/// (<c>POST /v1/auth/pin</c>, este projeto) é a única marcada no edge.
/// </para>
/// </remarks>
public sealed class IdempotencyMiddleware
{
    ...
    private async Task ExecuteAndStoreAsync(HttpContext context, IIdempotencyStore store, string key, CancellationToken cancellationToken)
    {
        ...
        {
            // US-156 (Nexora.Api.Cloud) — ver IdempotencyRedactFieldsAttribute: o que é GRAVADO
            // para um eventual reenvio pode divergir do que acabou de ser escrito em
            // `responseBytes` (a resposta AO VIVO desta chamada, já em voo abaixo) — só a cópia
            // persistida tem os campos marcados trocados por null. Nenhuma rota do Edge usa o
            // atributo hoje; mantido aqui só para os dois middlewares continuarem gêmeos.
