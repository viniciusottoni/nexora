...
/// a claim <c>ses</c> (login por PIN, edge) — sem <c>TenantId</c>/<c>SessionId</c> resolvidos (ex.:
/// os próprios endpoints de login), não há sessão nenhuma para checar, e a requisição segue normal.
/// Registrado depois de <c>UseAuthentication()</c>/antes de <c>UseAuthorization()</c>: sessão
/// inativa nega ANTES de qualquer policy de permissão avaliar (evita vazar, via a diferença entre
/// 401 e 403, se o operador teria ou não permissão para a rota).
/// </summary>
