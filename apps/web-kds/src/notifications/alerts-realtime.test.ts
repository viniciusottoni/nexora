...
describe('AlertsRealtimeClient (E-08 — porte de web-pos)', () => {
  it('invoca onAlert quando o hub emite alert', async () => {
    ...
    await client.start();
    emit('alert', { type: 'alert.raised', data: { alertId: 'alerta-1', alertType: 'ORDER_LATE' } });

    expect(onAlert).toHaveBeenCalledWith({ type: 'alert.raised', data: { alertId: 'alerta-1', alertType: 'ORDER_LATE' } });
  });
