...
└── Git/       monorepo (backend .NET + frontend React/Vite) — ver Build/lint/testes abaixo
    ...
    └── design system/   skill de design da Nexora (tokens, kits de UI, guidelines) — ver readme.md e SKILL.md dentro da pasta
```

O diretório de trabalho raiz **não** é um repositório git — apenas `Git/` é. `Docs/` (pacote de especificação) e o pacote de design system, que antes viviam fora do repositório, foram movidos para dentro de `Git/` como `docs/` e `design system/` (minúsculo) e agora estão versionados junto com o código; todas as referências de caminho neste arquivo usam `docs/` em minúsculo. Dentro de `Git/` existe a solution .NET (`backend/Nexora.slnx`, camadas do ADR-039) e o monorepo pnpm/turbo do frontend, com as 7 histórias do E-00 implementadas (US-001 a US-007) — mas **`Git/.git` ainda não tem nenhum commit**: todo esse código está no working tree, não versionado. Não assuma que o histórico de commits reflete o estado do código; confira o disco.
...
| **Soft delete sempre** (`deleted_at`); `DELETE` físico não existe (filtro global do EF Core por padrão). Auditoria e `domain_event` são append-only, com trigger que bloqueia mutação | `docs/domain/00` |
...
## Layout das telas de gestão (web-admin e web-platform)

Os dois painéis são o mesmo produto visto por dois perfis, então **a casca e a página vêm de um só lugar**: `packages/ui/src/components/console.css`. Tela nova de gestão não inventa layout — compõe as classes que já existem.

- **Casca**: `db-console` (flex, 100dvh, `overflow: hidden`) › `SideNav variant="dark"` de 248px + `db-console__main` › `TopBar` de 56px + `db-console__content` (o único que rola, gutter de 24px) › `db-console__column` (limitada a `--content-max`). São os dois únicos elementos de chrome fixo do produto; nada mais flutua sobre dado.
- **Página**: `db-page` › `db-page__header` (`db-page__eyebrow` 11px caps + `db-page__title` em `--type-h1` + `db-page__lead`) e o conteúdo em `Card`. Título de página é **28px Inter** — Montserrat (`--font-display`) é marca, capa e número-herói, nunca título de formulário.
- **Composições prontas**: `db-workbench` (lista + editor), `db-workbench--rail` (tabela + trilho), `db-stack`/`db-rail`, `db-list__item` (selecionado = `--surface-brand-subtle` + peso 600), `db-form-card`, `db-form-row`, `db-editor__footer`, `db-hint`, `db-loading`, `db-code`, `db-swatch`.
- **Toda tabela vive dentro de um `Card`** com título/subtítulo/ações (padrão do `ui_kits/admin-nexora`), e tabela escrita à mão reusa `db-table-wrap`/`db-table` em vez de recriar cabeçalho e linha. Aviso é `AlertBanner` — barra colorida à esquerda como decoração é proibida pelo Design System.
- Ícone é sempre o componente `Icon` (Material Symbols). `✓`, `•` ou emoji como ícone estão proibidos, inclusive em checklist.
...
- **Animação de entrada usa `backwards`, nunca `both`/`forwards`.** Uma animação de `transform` que continua "em vigor" pelo preenchimento faz o elemento virar **bloco contentor** de descendentes `position: fixed` — era o que recortava o véu do modal dentro do cartão que o abriu em vez de cobrir a tela. Como as entradas (`nx-slide-in`, `nx-scale-in`, `nx-toast-in`, `nx-open-x`) terminam no estado natural do elemento, `backwards` cobre o atraso do `.nx-stagger` e não deixa resíduo. `forwards` só onde o estado final difere do natural (ex.: `nx-brand-exit`, que termina invisível). Diálogo/popover, além disso, vai por **portal no `body`** (ver `packages/ui/src/components/modal.tsx`).
